...
def initiate_vector_store(database_path, nodes):
    ...
    # You can specify different models like "sentence-transformers/all-mpnet-base-v2"
    default_ef = HuggingFaceEmbedding(model_name="sentence-transformers/all-MiniLM-L6-v2")

    client = chromadb.PersistentClient(path=database_path)
    collection = client.get_or_create_collection(name="articles_hf_embed")
    vector_store = ChromaVectorStore(chroma_collection=collection)
    storage_context = StorageContext.from_defaults(vector_store=vector_store)
    index = VectorStoreIndex(nodes, storage_context=storage_context, embed_model=default_ef)
    print(f"Vector store created with embedding model: {default_ef.model_name}")
    return index 
